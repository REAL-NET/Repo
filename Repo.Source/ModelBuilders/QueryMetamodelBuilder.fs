...
type QueryMetamodelBuilder() =
    interface IModelBuilder with
        member this.Build(repo: IRepo): unit =
            ...
            let (~+) (name, xCoordinate, yCoordinate) =
                let node = infrastructure.Instantiate model metamodelNode :?> INode
                node.Name <- name
                infrastructure.Element.SetAttributeValue node "xCoordinate" xCoordinate
                infrastructure.Element.SetAttributeValue node "yCoordinate" yCoordinate
